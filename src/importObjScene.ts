...
import ObjLoader from "./obj/Parser";
import { MeshObject } from "./obj/MeshObj";
...
async function init() {
  ...
  const objContent = await ObjLoader.load("../assets/indoorplant.obj");
  const mtlContent = await ObjLoader.loadMtl("../assets/indoorplant.mtl");
  const geometry = await ObjLoader.parse(objContent, mtlContent);
  

 

  GeometryBuffersCollection.initialize(device);
  GeometryBuffersCollection.initializeObjects(device, geometry);
  ...
  shadowCamera.eye = new Vec3(5.56, 5.95, -6.293); // Let's imagine it as a negative direction light * -20 or any other fitting scalar
  shadowCamera.target = new Vec3(5.0, 5.45, -5.62);
  ...
  // Extract joints from the GLTF file
  const obj = new MeshObject(device, camera, shadowCamera, ambientLight, directionalLight, pointLights);
  obj.pipeline.shadowTexture = shadowTexture;

  obj.position = new Vec3(0, -1.8, 0);
  ...
  const shadowPass = (commandEncoder: GPUCommandEncoder) => {
    ...
    obj.drawShadows(renderPassEncoder); 
