...
export class Circle{
    ...
    constructor(device: GPUDevice, camera: Camera, shadowCamera: ShadowCamera, ambientLight: AmbientLight, directionalLight: DirectionalLight,  pointLights: PointLightsCollection){
        this.transformBuffer = new UniformBuffer(device, this.transform, "Circle Transform");

        this.normalMatrixBuffer = new UniformBuffer(device, 16 * Float32Array.BYTES_PER_ELEMENT, "Circle Normal Matrix");
    ...
    public drawShadows(renderPassEncoder: GPURenderPassEncoder){
        if(!this.visible) return;
        this.shadowPipeline.draw(renderPassEncoder, GeometryBuffersCollection.circleBuffers);
    }
