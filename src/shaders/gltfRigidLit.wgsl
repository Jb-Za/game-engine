...
@group(3) @binding(2) var<uniform> diffuseColor: vec4f;
@group(3) @binding(3) var<uniform> shininess: f32;
@group(3) @binding(4) var shadowTexture: texture_depth_2d;
...
fn fragmentMain(input: VertexOutput) -> @location(0) vec4f {
    ...
    switch general_uniforms.render_mode {
        case 3: {
            // Lit mode - full lighting calculations
            // Shadows
            ...
            for(var i = 0; i < 3; i++) {
                ...
                var pointDotSpecular = max(dot(normal, pointHalfVector), 0.0);
                pointDotSpecular = pow(pointDotSpecular, shininess);
                lightAmount += positionalLight[i].specularColor * pointDotSpecular * positionalLight[i].specularIntensity * attenuation * shadowFactor;
            }
