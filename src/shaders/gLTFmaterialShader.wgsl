...
@group(0) @binding(3)
var<uniform> globalBoneTransform : array<mat4x3f, 100>;
...
fn applyBoneTransform(p : vec4f, boneIDs : vec4f, weights : vec4f) -> vec3f {
    var result : vec3f = vec3f(0.0);
    for (var i : u32 = 0u; i < 4u; i = i + 1u)
    {
        let boneIndex : u32 = u32(boneIDs[i]);
        let boneTransform : mat4x3f = globalBoneTransform[boneIndex];
        result = result + weights[i] * (boneTransform * p).xyz;
    }
    return result;
}
...
{
    var out : VSOutput;
    let position = applyBoneTransform(vec4f(in.position, 1.0), in.boneIDs, in.weights);
    let normal = normalize(applyBoneTransform(vec4f(in.normal, 0.0), in.boneIDs, in.weights));

    out.position = projectionView * transform[iid] * vec4f(position, 1.0);
    out.color = in.color;
    out.texCoord = in.texCoord * texturetiling;
    out.normal = normalMatrix[iid] * normal;
    out.fragPos = (transform[iid] * vec4f(position, 1.0)).xyz;
    out.eye = eye;
