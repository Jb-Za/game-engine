...
export class RayTracingRenderPipeline {
    ...
    public updateCamera() {
        ...
        cameraData[3] = 0.0; // _pad0
        // Forward vector (from eye to target)
        const forward = Vec3.normalize(Vec3.subtract(this.camera.target, this.camera.eye));
    ...
    public updateSpheres(spheres: RayTracedSphere[]) {
        if (this.spheres != spheres) {
            ...
            console.log(`Updating ${spheres.length} spheres:`, spheres.map((s, i) =>
                `Sphere ${i}: pos(${s.center.x}, ${s.center.y}, ${s.center.z}), radius=${s.radius}, color(${s.material.color.x}, ${s.material.color.y}, ${s.material.color.z})`
            ));
